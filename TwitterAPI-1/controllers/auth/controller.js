...
exports.protect=async (req,res,next)=>{
    try {
        ...
        return next(new errorObj("Not Logged in..",403));
        ...
        //check if the user with the id from the payload exists

        const user=await userModel.findById(payLoad.id);

        if(!user)return next(new errorObj("Invalid Credentials.Log in again",400));

        // return res and call next
        req.user=user;
        next();
...
exports.login=async (req,res,next)=>{
    ...
    try {
        ...
        res.cookie('jwt',token,{expiresIn:process.env.JWT_EXP,httpOnly:true});
